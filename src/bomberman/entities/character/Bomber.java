...
public class Bomber extends Character {
    ...
    protected void calculateMove() {
        // xử lý nhận tín hiệu điều khiển hướng đi từ input và gọi move() để thực hiện di chuyển
        int x1 = 0, y1 = 0;
    ...
    public void move(double xa, double ya) {
        // sử dụng canMove() để kiểm tra xem có thể di chuyển tới điểm đã tính toán hay không và thực hiện thay đổi tọa độ coordinateX, coordinateY
        // nhớ cập nhật giá trị direction sau khi di chuyển
        if (xa < 0) {
            direction = 3;
        }
        if (xa > 0) {
